...
def get_housing_predictions_chart(df: pd.DataFrame, district_name: str) -> alt.Chart:
    ...
    df_district = df_district[["Year", "Pred_Num_Apartments"]]
    df_district["Pred_Num_Apartments"] = df_district["Pred_Num_Apartments"].astype(int)
    ...
    chart += alt.Chart(df_district).mark_line(opacity=0.3).encode(
        x='Year',
        y = alt.Y('Pred_Num_Apartments_Low_CI', title = None),        #y2 = alt.Y2(f'{district_name}_High_CI', axis=alt.Axis(labels=False)),
    )
    chart += alt.Chart(df_district).mark_line(opacity=0.3).encode(
        x='Year',
        y = alt.Y(f'Pred_Num_Apartments_High_CI', title = None),        #y2 = alt.Y2(f'{district_name}_High_CI', axis=alt.Axis(labels=False)),
        #y2 = alt.Y2(f'{district_name}_High_CI', axis=alt.Axis(labels=False)),
    )

    # Customize the chart appearance
    chart = chart.properties(
        title=f'{district_name} No. Apartments Over Time',
        width=500,
        height=300, 
    ).encode(
        y=alt.Y(f'Pred_Num_Apartments', title='No. Apartments')
    )
