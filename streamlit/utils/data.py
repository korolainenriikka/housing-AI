...
def get_populations_data() -> Dict:
    dataframes = {}
    files = os.listdir("./streamlit/data/gold/populations")
    for file in files:
        district_name = file.split("_")[0]
        df = pd.DataFrame(
            pd.read_csv(f'./streamlit/data/gold/populations/{file}').rename(columns={'Unnamed: 0': 'Year'})
        )
...
def get_total_population_data() -> pd.DataFrame:
    df = pd.read_csv("./streamlit/data/gold/total/total_population_per_district.csv", index_col=0)
    return df
